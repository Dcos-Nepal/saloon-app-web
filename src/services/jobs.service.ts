...
export const fetchJobsApi = async (query: Record<string, any>) => {
  const url = '/v1/jobs' + (query ? `?${generateQueryParams(query)}` : '');
...
export const getJobsSummaryApi = async (query: any) => {
  const url = '/v1/jobs/summary' + (query ? `?${generateQueryParams(query)}` : '');
  return await http.get(url, {
    headers: { Accept: 'application/json' }
  });
};
