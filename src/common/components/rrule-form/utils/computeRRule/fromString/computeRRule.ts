...
const computeRRule = (data: any, rrule: any) => {
  if (!rrule) {
    return data;
  }
  ...
  try {
    ...
    newDataObj = {
      ...
      start: {
        ...data.start,
        onDate: {
          date: format(computeStartOnDate(data, rruleObj), DATE_TIME_FORMAT),
          options: {
      ...
      repeat: {
        ...
        onDate: {
          date: format(new Date(computeEndOnDate(data, rruleObj)), DATE_TIME_FORMAT),
          options: {
